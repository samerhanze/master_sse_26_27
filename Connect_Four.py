...
def play_turn(board, col, piece):
    global game_over
    global turn
    if board[ROWS-1][col]==0: #check that top row is not full
...
while not game_over:
    ...
    if turn == 0:
        ...
        col = int(input(f"Player 1, Make your Selection(0-{COLUMNS-1}):"))
        play_turn(board, col, piece)
         
    #Player 2's turn
    else:
        piece = 2 #Piece for Player 2
        col = int(input(f"Player 2, Make your Selection(0-{COLUMNS-1}):"))
        play_turn(board, col, piece)
